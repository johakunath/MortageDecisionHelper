...
**Consequence.**
- The Verlauf chart is focusable and walks with ←/→ (Home/End/Esc). Two people at one screen do not always share a mouse, and the repo already treats hover-only affordances as a defect — `InfoTip` opens on hover *and* focus for the same reason.
