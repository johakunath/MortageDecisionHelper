...
- **`.app-header` must stay a fixed height.** It sets `height: var(--header-height)` explicitly, and every sticky element below clears it with that same constant — if the two disagree, a band of scrolling content shows through the seam ([D8](docs/DECISIONS.md)). Nothing collapsible and nothing that grows with its content goes in it. It carries context (brand, active apartment, price) plus **one** control, the EK switch, which is fixed-height by construction and earns the space because it is the app's central question ([D21](docs/DECISIONS.md), [D25](docs/DECISIONS.md)). Adding anything here means re-measuring the header at 1280 / 1000 / 820 / 560px — it must read exactly `--header-height` at all four. The topbar never stacks; at narrow widths it drops content instead, and the apartment's name and the switch survive every width.
